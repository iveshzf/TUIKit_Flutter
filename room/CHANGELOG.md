...
### 4.0.0(2026-01-07)
> ⚠️ **Breaking Change**: This is a major breaking update. Existing users are not recommended to upgrade.

- **Architecture Refactoring**: Rebuilt using `atomic_x_core` library, adopting a Store/State architecture pattern for state management. Each Store exposes reactive `State` objects with `ValueNotifier` for UI binding.
- **Supported Features**:
  - Basic room audio/video functionality.
  - Member management (kick, mute microphone, disable camera, admin assignment).
  - All members control (mute all microphones, disable all cameras).
  - Device control (camera, microphone, speaker toggle).
  - Real-time participant status updates.
- **New API**:
  - Introduced `RoomMainWidget` as the main entry point.
  - Added `RoomBehavior.create()` and `RoomBehavior.enter()` for room creation and joining.
  - Added `ConnectConfig` for configuring auto-enable camera, microphone, and speaker.
