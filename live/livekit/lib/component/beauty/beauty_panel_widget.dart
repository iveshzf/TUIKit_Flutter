...
class _BeautyPanelWidgetState extends State<BeautyPanelWidget> {
  ...
  late final BaseBeautyStore _baseBeautyStore;

  @override
  void initState() {
    super.initState();
    _baseBeautyStore = BaseBeautyStore.shared;
    _initData();
  }
...
extension on _BeautyPanelWidgetState {
  void _onSliderChanged(double value) {
    ...
    if (selectIndex.value == 1) {
      _baseBeautyStore.setSmoothLevel(sliderValue.value.toDouble());
    } else if (selectIndex.value == 2) {
      _baseBeautyStore.setWhitenessLevel(sliderValue.value.toDouble());
    } else if (selectIndex.value == 3) {
      _baseBeautyStore.setRuddyLevel(sliderValue.value.toDouble());
    }
  }
  ...
  void _onTapIndex(int index) {
    ...
    switch (item.type) {
      ...
      case BeautyItemType.smooth:
        sliderValue.value = _baseBeautyStore.baseBeautyState.smoothLevel.value.toInt();
        break;
      case BeautyItemType.whiteness:
        sliderValue.value = _baseBeautyStore.baseBeautyState.whitenessLevel.value.toInt();
        break;
  ...
  bool _isClose() {
    final state = _baseBeautyStore.baseBeautyState;
