...
public class TuilivekitPlugin implements FlutterPlugin, MethodCallHandler, ActivityAware {
    ...
    private PictureInPictureManager mPipManager;

    private final Application.ActivityLifecycleCallbacks mActivityCallback = new EmptyActivityLifecycleCallbacks() {
        @Override
        public void onActivityPaused(@NonNull Activity activity) {
            Log.i(TAG, "onActivityPaused:" + activity);
            if (mActivity == activity) {
                mPipManager.enterPictureInPicture(activity);
            }
        }
    ...
    public void onAttachedToEngine(@NonNull FlutterPluginBinding flutterPluginBinding) {
        ...
        mPipManager = new PictureInPictureManager();
        mPipEventChannel = new EventChannel(flutterPluginBinding.getBinaryMessenger(), "tuilivekit_pip_events");
        mPipEventChannel.setStreamHandler(mPipManager);
    }
    ...
    public void onDetachedFromEngine(@NonNull FlutterPluginBinding binding) {
        ...
        mPipEventChannel.setStreamHandler(null);
    }
    ...
    public void enablePictureInPicture(MethodCall call, MethodChannel.Result result) {
        ...
        String params = MethodUtils.getMethodRequiredParams(call, "params", result);
        result.success(mPipManager.enablePictureInPicture(activity, params));
    }

    public void exitPictureInPicture(MethodCall call, MethodChannel.Result result) {
        Activity activity = mActivity;
        if (activity != null) {
            mPipManager.exitPictureInPicture(activity);
        }
    }
}
